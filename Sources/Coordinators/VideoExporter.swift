...
public class VideoExporter {
  ...
  public func export(completion: @escaping (Result<Void, ExportError>) -> Void) {
    ...
    guard let cameraVideoTrack = cameraAsset.tracks(withMediaType: AVMediaType.video).first else {
      completion(.failure(.missingCameraVideo(at: cameraVideoURL)))
      return
    }

    guard let cameraAudioTrack = cameraAsset.tracks(withMediaType: AVMediaType.audio).first else {
      completion(.failure(.missingCameraAudio(at: cameraVideoURL)))
      return
    }

    guard let screenVideoTrack = screenAsset.tracks(withMediaType: AVMediaType.video).first else {
